...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    {
        new Thread(new Bob(0,0,"red",400)).start();
        new Thread(new Bob(150, 150, "blue",300)).start();
        new Thread(new Bob(-125, 125, "green",250)).start();
        new Thread(new Bob(-40,100,"yellow",280)).start();
        new Thread(new Bob(190,0,"pink",270)).start();
       // Turtle bob = new Turtle();
        //Turtle boby = new Turtle();



        // john.saveGCODE("test.gcode");
    }
